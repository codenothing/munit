...
Assert.prototype = {
	...
	requireState: function( required, startFunc ) {
		...
		if ( required !== self.state ) {
			self._stateError( startFunc, startFunc || self._requireState );
		}
	...
	requireMaxState: function( max, startFunc ) {
		...
		if ( max < self.state ) {
			self._stateError( startFunc, startFunc || self.requireMaxState );
		}
	...
	requireMinState: function( min, startFunc ) {
		...
		if ( min > self.state ) {
			self._stateError( startFunc, startFunc || self.requireMinState );
		}
	...
	module: function( name, options, callback ) {
		...
		// Module creation is done on the main object
		return munit.module( self.nsPath + '.' + name, options, callback );
	},
