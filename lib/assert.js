...
Assert.prototype = {
	...
	module: function( name, options, callback ) {
		...
		return munit._module( name, options, callback, self.nsPath );
	},
	...
	junit: function(){
		var self = this,
			...
			xml = "<testsuite name='" + munit._xmlEncode( prefix + self.nsPath ) + "' tests='" + self.count + "' failures='" + self.failed + "' skipped='" + self.skipped + "' time='" + ( ( self.end - self.start ) / 1000 ) + "'>";
	...
	close: function( startFunc, forced ) {
		...
		self._spies.forEach(function( spy ) {
			spy.restore();
		});
...
AssertResult.prototype = {
	...
	junit: function(){
		...
		return "<testcase name='" + munit._xmlEncode( self.name ) + "' classname='" + munit._xmlEncode( prefix + self.nsPath ) + "' time='" + ( self.time / 1000 ) + "'>" + message + "</testcase>";
	},
