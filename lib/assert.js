...
Assert.prototype = {
	...
	_teardown: function( callback ) {
		...
		if ( self.options.teardown ) {
			self.options.teardown( self, function teardownCallback(){
				// Added precaution
	...
	trigger: function(){
		...
		self._setup(function( e, setupCallback ) {
			if ( e ) {
				self.fail( "Failed to setup module", setupCallback, e );
				return self.close();
			}
	...
	close: function( startFunc, forced ) {
		...
		if ( self.count < 1 ) {
			self.fail( "No tests ran in this module" );
		}
	...
	_close: function( startFunc, forced ) {
		...
		else if ( self.count < 1 && munit.render.focusPath( self.nsPath ) ) {
			self._fail(
				"No Tests Found",
				startFunc || self._close,
