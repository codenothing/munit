...
function color(){
	...
	if ( munit.isArray( value ) && ( process.stdout.isTTY || process.env.NODE_COLOR_OUTPUT ) ) {
...
munit.each( colors, function( value, name ) {
	...
	color[ name ] = function(){
		munit.log( color.get[ name ].apply( color.get, arguments ) );
	};
});

// Plain output
color.plain = color[ 'default' ] = function(){
	munit.log.apply( munit, Slice.call( arguments ) );
};
