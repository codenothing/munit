...
function render( path, options, callback ) {
	...
	if ( munit.isString( render.options.render ) ) {
		render._setup();
	}
	else {
		render._compile();
...
munit.extend( render, {
	...
	option: function( name, value ) {
		...
		if ( munit.isObject( name ) ) {
			return munit.each( name, function( value, name ) {
				render.option( name, value );
			});
		}
		// Requesting to get the value of an option
		else if ( value === undefined ) {
			return render.options[ name ];
		}

		// Disallow changing of options past the read state
		// This will help inform devs of why their options don't work
		render.requireMaxState( munit.RENDER_STATE_READ, render.option );

		// Extra error handling for render option
		// Again, prevents nothing internaly, just to help devs
		if ( name == 'render' && render.state !== munit.RENDER_STATE_DEFAULT ) {
			throw new Error( "munit is already rendering, can't change render path" );
		}

		render.options[ name ] = value;
		return;
	},
