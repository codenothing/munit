...
munit.extend({
	...
	// Module creation
	_module: function( name, options, callback, _nsprefix ) {
		var found = false;

		// NS Prefix handling
		if ( munit.isString( _nsprefix ) && _nsprefix.length ) {
			_nsprefix += '.';
		}
		else {
			_nsprefix = "";
		...
		// Block if tests are already running
		...
		// munit( name, { options }, { modules });
			...
			return munit.each( callback, function( callback, sub ) {
				munit._module( _nsprefix + name + '.' + sub, options, callback );
			});
		}
		// Test for object of options/modules
			...
			if ( found ) {
				return munit.each( options, function( callback, sub ) {
					munit._module( _nsprefix + name + '.' + sub, undefined, callback );
				});
			}
		...
		return munit._createModule( _nsprefix + name, options, callback );
	},
	...
	async: function( name, options, callback ) {
		if ( callback === undefined ) {
			...
			options = { isAsync: true, expect: options };
		}
